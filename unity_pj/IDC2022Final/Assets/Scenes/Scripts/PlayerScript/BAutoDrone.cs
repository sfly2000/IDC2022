...
public class BAutoDroneCtrl : MonoBehaviour
{
    ...
    public Vector3 [] trees = new Vector3[4];
    public Vector3 tar = new Vector3();
    public Vector3 tar1 = new Vector3();
    public float ang;
    ...
    public class SetInputKey
    {
        ...
        public string InputKeyAutoStraight;
        public string low;
        public string high;
    }
    ...
    private void Update()
    {
        if (PhotonNetwork.CurrentRoom == null || photonView.IsMine)
        {
            ...
            if (Input.GetKeyDown(InputKey.Next))
            {
                num += 1;
                Debug.Log(num);
            }
            if (Input.GetKeyDown(InputKey.Prefer))
            {
                num -= 1;
                Debug.Log(num);
            }
    ...
    void FixedUpdate()
    {
        if (PhotonNetwork.CurrentRoom == null || photonView.IsMine)
        {
            ...
            if (Input.GetKey(InputKey.AutoAim))
            {
                float dif = Ang();
                HorizontalRotationControl.set_eular_y += dif;
            }

            if (Input.GetKey(InputKey.InputKeyAutoStraight))
            {
                Vector3 v = trees[num] - transform.position;
                v.y = 0f;
                expect_velocity_z = Mathf.Min(v.magnitude/4f, DipControl.set_vz);
            }
            if (Input.GetKey(InputKey.low))
            {
                HeightControl.set_height = 0.5f;
            }
            if (Input.GetKey(InputKey.high))
            {
                HeightControl.set_height = 4f;
            }
            //****************************************************************************************************************************
    ...
    float Ang()
    {
        tar = trees[num] - transform.position;
        tar.y = 0f;
        if (tar.x <= 0f)
        {
            ang = 360 - Vector3.Angle(tar, tar1);
        }
        else
        {
            ang= Vector3.Angle(tar, tar1);
        }
        return ang - HorizontalRotationControl.set_eular_y;
    }

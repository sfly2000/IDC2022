...
using System;
public class SevenA_Droneold : MonoBehaviour
{
    ...
    public float origin_mass = 1.8f + 1.4f;
    public float now_mass = 1.8f + 1.4f;
    //9.81*weight/4
    ...
    void FixedUpdate()
    {
        ...
        if (PhotonNetwork.CurrentRoom == null || photonView.IsMine)
        {
            ...
            if (is_rigid)
            {
                ...
                Pump.pump2.AddRelativeForce(vector * Pump.force * dir2 * (1 + Pump.gain));
            }
            //****************************************************************************************************************************
            ...
            else if (Input.GetKey(InputKey.InputKeyLeft))
            {
                ...
                //修改
                //不能到
                Target.expect_position_x = Mathf.Max(Target.expect_position_x, 0);
            }
            ...
            else if (Input.GetKey(InputKey.InputKeyBackward))
            {
                ...
                //修改
                //不能到
                Target.expect_position_z = Mathf.Max(Target.expect_position_z, 0);
            }
            //float delta_force = 1f;
            /*
            //前后运动
            //这里3f相当于让它的加速度为3
            if (Input.GetKey(InputKey.InputKeyForward))
            {//相当于让它不超过最大速度
                ...
                expect_velocity_z = Mathf.Min(expect_velocity_z + DipControl.want_acc * Time.deltaTime, DipControl.set_vz);
            }
            else if (Input.GetKey(InputKey.InputKeyBackward))
            {//相当于让它不超过最大速度
                //PosForce.LFforce += delta_force;
                ...
                //PosForce.RFforce += delta_force;
                //PosForce.RBforce += -delta_force;
                expect_velocity_z = Mathf.Max(expect_velocity_z - DipControl.want_acc * Time.deltaTime, -DipControl.set_vz);
            }
            else//直到回归0的停止状态
            {
                if (expect_velocity_z > 0)
                {
                    expect_velocity_z = Mathf.Max(expect_velocity_z - DipControl.want_acc * Time.deltaTime, 0);
                }
                else if (expect_velocity_z < 0)
                {
                    expect_velocity_z = Mathf.Min(expect_velocity_z + DipControl.want_acc * Time.deltaTime, 0);
                }
            ...
            */
            ...
            if (Input.GetKey(InputKey.InputKeyForward))
            {//向前飞1m
                Target.expect_position_z = position_old.z + 10f;
            }
            else if (Input.GetKey(InputKey.InputKeyBackward))
            {//相当于让它不超过最大速度
                Target.expect_position_z = position_old.z - 10f;
            ...
            */
            ...
            if (Input.GetKey(InputKey.InputKeyRight))
            {
                ...
                //PosForce.RFforce += -delta_force;
                //PosForce.RBforce += -delta_force;
                expect_velocity_x = Mathf.Min(expect_velocity_x + DipControl.want_acc * Time.deltaTime, DipControl.set_vx);
            }
            else if (Input.GetKey(InputKey.InputKeyLeft))
            {
                //PosForce.LFforce += -delta_force;
                ...
                //PosForce.RFforce += delta_force;
                //PosForce.RBforce += delta_force;
                expect_velocity_x = Mathf.Max(expect_velocity_x - DipControl.want_acc * Time.deltaTime, -DipControl.set_vx);
            }
            else
            {
                if (expect_velocity_x > 0)
                {
                    expect_velocity_x = Mathf.Max(expect_velocity_x - DipControl.want_acc * Time.deltaTime, 0);
                }
                else if (expect_velocity_x < 0)
                {
                    expect_velocity_x = Mathf.Min(expect_velocity_x + DipControl.want_acc * Time.deltaTime, 0);
                }
            }
            */
            /*
            if (Input.GetKey(InputKey.InputKeyRight))
            {//向前飞1m
                Target.expect_position_x = position_old.x + 10f;
            }
            else if (Input.GetKey(InputKey.InputKeyLeft))
            {//相当于让它不超过最大速度
                Target.expect_position_x = position_old.x - 10f;

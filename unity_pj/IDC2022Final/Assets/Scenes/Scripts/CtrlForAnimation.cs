...
//四翼无人机控制版本3 07 04
//特点：自动实现高度控制；基于速度的PID控制实现前进后退
public class CtrlForAnimation : MonoBehaviour
{
    ...
    public float set_vx_now = 0f;
    public float set_vz_now = 0f;
    public float set_height_now;
    public float set_eular_y_now;

    public bool is_working;
    public float original_force = 8f;
    ...
    {
        public float set_vx = 5;
        public float set_vz = 5;
    ...
    {
        //private float[] px = { 0.0102f, 0.0305f, 0.0305f, 0.0102f };
        //private float[] py = { -2.004f, 1.447f, -0.362f };
        public float[] px1 = { 0.00475f, 0.01425f, 0.01425f, 0.00475f };
        public float[] py1 = { -2.25f, 1.756f, -0.4683f };
        //public float[] px2 = { 0.000115f, 0.000345f, 0.000345f, 0.000115f };
        //public float[] py2 = { -2.8f, 2.618f, -0.818f };
    ...
    void FixedUpdate()
    {
        if (photonView.IsMine)
        {
            ...
            {
                HeightControl.set_height -= HeightControl.set_vy * Time.deltaTime;
                HeightControl.set_height = Mathf.Max(HeightControl.set_height, 0);
            }
            ...
            //PID参数计算
            float des_acceleration_z = Mathf.Clamp(1.2f * (set_vz_now - velocity_z_now), -5, 5);
            ...
            //PID参数计算
            float des_acceleration_x = Mathf.Clamp(1.2f * (set_vx_now - velocity_x_now), -5, 5);
            ...
            //****************************************************************************************************************************
            //倾角控制
            //当前版本的欧拉角的取值范围时0~360度，因此在跨约360度时会发生突变，因此使用Mathf.DeltaAngle计算角度差
            ...
            //****************************************************************************************************************************
            //转角控制
            //HorizontalRotationControl.set_eular_y = testparameter;
            float delta_eular_y = Mathf.DeltaAngle(rig.transform.eulerAngles.y, set_eular_y_now);
            ...
            //****************************************************************************************************************************
            //高度控制
            float delta_height = set_height_now - rig.transform.position.y;
            ...
            //****************************************************************************************************************************
            //设定飞行器升力与力矩（请不要修改这一部分）
            LFforce = Mathf.Clamp(LFforce, 0f, 20f);
            LBforce = Mathf.Clamp(LBforce, 0f, 20f);
            RFforce = Mathf.Clamp(RFforce, 0f, 20f);
            RBforce = Mathf.Clamp(RBforce, 0f, 20f);
